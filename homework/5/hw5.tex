...
\begin{enumerate}
 ...
 \item Proofs
  ...
  \item \textbf{Theorem} \quad If $\varnothing \,|\, \Sigma \vdash t : T$, $\varnothing \,|\, \Sigma \vdash \mu$, and $t \,|\, \mu \to t^\prime \,|\, \mu^\prime$, then there exists a $\Sigma^\prime$ such that $\Sigma^\prime \supseteq \Sigma$, $\varnothing \,|\, \Sigma^\prime \vdash t^\prime : T$, and $\varnothing \,|\, \Sigma^\prime \vdash \mu^\prime$.\\
    \textbf{Proof} \quad By induction on the derivation of $\varnothing \,|\, \Sigma \vdash t : T$. Case analysis of the last rule used in the derivation.
      \begin{itemize}
      \item Case \textsc{T-Ref}: Then $t = \mathtt{ref}\ t_1$, $\varnothing \,|\, \Sigma \vdash t_1 : T_1$, and $T = \mathtt{Ref}\ T_1$. Case analysis on the last rule used in the derivation of $t \,|\, \mu \to t^\prime \,|\, \mu^\prime$.
        \begin{itemize}
        \item Case \textsc{E-Ref}: Then $t_1 \,|\, \mu \to t_1^\prime \,|\, \mu^\prime$ and $t^\prime = \mathtt{ref}\ t_1^\prime$. By the induction hypothesis, there exists a $\Sigma^\prime$ such that $\Sigma^\prime \supseteq \Sigma$, $\varnothing \,|\, \Sigma^\prime \vdash t_1^\prime : T_1$, and $\varnothing \,|\, \Sigma^\prime \vdash \mu^\prime$. Then $\varnothing \,|\, \Sigma^\prime \vdash \mathtt{ref}\ t_1^\prime : \mathtt{Ref}\ T_1$ by \textsc{T-Ref}.
        \item Case \textsc{E-RefV}: Then $t_1$ is a value $v_1$ with type $T_1$, $t^\prime$ is a location $\ell \notin \text{dom}(\mu)$, and $\mu^\prime = \mu \cup \{ \ell \mapsto v_1 \}$. We know that $\varnothing\ |\ \Sigma' \vdash \mu'$ means $\Sigma' \supseteq \Sigma$ because $l \notin dom(\mu)$, and we know that a $\varnothing\ |\ \Sigma' \vdash t' : T$ (which is $\mathtt{Ref}\ T_1$) by T-Loc and the type for $v_1$ which is $T_1$.
        \end{itemize}
      \end{itemize}
  \end{enumerate}
...
\item Freeing Refs
  ...
  \item $\mathtt{let}\ x = \mathtt{ref}\ 0\ \mathtt{in\ free}\ x;\ \mathtt{free}\ x$
